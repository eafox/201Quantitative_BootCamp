...
names(snpsDataFrame)
#individuals each have their own columns

# What are the row names? 
row.names(snpsDataFrame)
#SNPs have their own rows (the boxes are how many of that SNP each individual has)
...
table(testSNP)
#Counts number of individuals for each haplotype
#can pull specific levels out

# What is proportion of heterozygotes at this locus?
het=sum(testSNP==1)/length(testSNP)
het
#tesSNP==1, those with that value will be true and used, others will be falses and ignored
#sum trues and divide by total
...
# Try these commands
table(testSNP)
#doesn't sum to righ amount
testSNP==1
length(testSNP)
is.na(testSNP)
#tells us if there is missing data
#could test if there were any by summing and seeing if it was greater than 0
...
het
#remove true values in sum and remove them when counting the whole data set
...
freq=sum(testSNP,na.rm=TRUE)/(2.0*sum(!is.na(testSNP))) ; freq
#sum number of copies and divide by total
...
#calculates each SNPs freq (one per row) and heterozygosity
...
#increase in frequency increase heterozygosity 
